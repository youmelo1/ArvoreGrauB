...
public class Menu {
		...
		private File file = new File("E:\\Usu�rios\\User1\\Desktop\\ArvoreAVLFinal\\src\\test.txt");
		...
		// O retorno da função retorna � o array de objetos 
		...
		public void consultarPorData() {
			...
			/*List<Integer> indicesEncontrados = arvoreDatas.listaIndices(dataInicio, dataFim);
			
			if(indicesEncontrados.size() != 0){
				for(int index : indicesEncontrados) {
					l.retornaDados(index);
			}else{
